...
test('configuração do Asaas usa HTTPS, valor e trial limitado a 15 dias', () => {
  const config = configurarAsaas({ ASAAS_API_KEY:'chave', ASAAS_BASE_URL:'https://api.asaas.com/v3/', NORDEN_PLANO_VALOR:'11000', NORDEN_TRIAL_DIAS:'30' });
  assert.equal(config.baseUrl, 'https://api.asaas.com/v3');
  assert.equal(config.valorCentavos, 11000);
...
test('resumo mostra valor em reais e dias restantes do teste grátis', () => {
  ...
  const resumo = resumoAssinatura({ status:'trial', valor_centavos:11000, trial_fim_em:amanha.toISOString() });
  assert.equal(resumo.valor_reais, 110);
