...
test('conclui embedded signup buscando WABA pelo token quando o popup não informa IDs', async () => {
  const chamadas = [];
  const consultar = async (url, opcoes = {}) => {
    ...
    if (url.includes('/debug_token?')) return { ok:true, json:async () => ({
      data:{ granular_scopes:[{ scope:'whatsapp_business_management', target_ids:['987654'] }] },
    }) };
    if (url.includes('/987654/phone_numbers')) return { ok:true, json:async () => ({
      data:[{ id:'123456789', display_phone_number:'+55 11 99999-0000' }],
    }) };
    if (url.includes('/987654/subscribed_apps')) return { ok:true, json:async () => ({ success:true }) };
    throw new Error(`Chamada inesperada: ${url}`);
  };
  ...
  const whatsapp = await concluirEmbeddedSignup({
    banco,
    empresa:{ id:5 },
    body:{ code:'code-meta', redirect_uri:'https://norden-a0bb.onrender.com/' },
    consultar,
    env:{ META_APP_ID:'app', META_APP_SECRET:'secret', META_EMBEDDED_SIGNUP_CONFIG_ID:'config', WHATSAPP_API_VERSION:'v26.0' },
  });
