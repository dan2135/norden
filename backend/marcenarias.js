...
async function resolverMarcenaria(req, res, next, banco) {
  try {
    ...
    if (!informado) throw erroHttp(400, 'Selecione uma empresa para continuar.');
    const resultado = req.usuario.superadministrador
      ? await banco.query(`SELECT id,nome,slug,segmento,atividade,'superadministrador' AS papel FROM marcenarias WHERE id=$1 AND ativa=TRUE`, [validarId(informado)])
      : await banco.query(`SELECT m.id,m.nome,m.slug,m.segmento,m.atividade,mm.papel FROM marcenarias m
    ...
    if (!resultado.rows[0]) throw erroHttp(404, 'Empresa não encontrada ou inativa.');
...
function registrarMarcenarias(app, banco, rota) {
  app.get('/api/marcenarias', rota(async (req, res) => {
    const resultado = req.usuario.superadministrador
      ? await banco.query("SELECT id,nome,slug,segmento,atividade,'superadministrador' AS papel FROM marcenarias WHERE ativa=TRUE ORDER BY nome,id")
      : await banco.query(`SELECT m.id,m.nome,m.slug,m.segmento,m.atividade,mm.papel FROM membros_marcenaria mm
  ...
  app.post('/api/marcenarias', rota(async (req, res) => {
    ...
    const atividade = typeof req.body?.atividade === 'string' ? req.body.atividade.trim() : '';
    if (segmento === 'outros' && !atividade) throw erroHttp(400, 'Informe o ramo da empresa.');
