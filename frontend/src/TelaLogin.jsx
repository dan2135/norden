...
export default function TelaLogin({ onEntrar }) {
  ...
  const [modo, setModo] = useState(parametros.has('redefinir') ? 'redefinir' : parametros.has('confirmar') ? 'confirmar' : 'login');
  const [form, setForm] = useState({ nome: '', email: '', identificador: '', senha: '', documento: '', empresa: '', segmento: 'outros' });
  ...
  useEffect(() => {
    const token=parametros.get('confirmar'); if(!token)return;
    requisicao('/auth/confirmar-email',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token})})
      ...
      .catch(e=>setEstado({enviando:false,erro:e.message}));
  ...
  async function enviar(event) {
    ...
    try {
      const caminho = primeiroAcesso ? '/auth/configurar' : modo==='cadastro'?'/auth/cadastro':modo==='esqueci'?'/auth/esqueci-senha':modo==='redefinir'?'/auth/redefinir-senha':'/auth/login';
      ...
      if(modo!=='login'&&!primeiroAcesso){setSucesso(sessao.mensagem);setModo('login');setEstado({enviando:false,erro:''});history.replaceState({},'',location.pathname);return;}
  ...
  return <main className="login-pagina">
    ...
    <section className="login-area">
      <form className="login-card" onSubmit={enviar}>
        ...
        <p className="sobretitulo">{primeiroAcesso ? 'CONFIGURAÇÃO INICIAL' : modo==='cadastro'?'NOVO CADASTRO':modo==='esqueci'?'RECUPERAR ACESSO':modo==='redefinir'?'NOVA SENHA':'ÁREA DO GESTOR'}</p>
        <h2>{primeiroAcesso||modo==='cadastro'?'Crie seu acesso':modo==='esqueci'?'Esqueceu sua senha?':modo==='redefinir'?'Escolha uma nova senha':'Bem-vindo de volta'}</h2>
        <p className="login-ajuda">{modo==='esqueci'?'Enviaremos um link de recuperação ao e-mail cadastrado.':modo==='redefinir'?'Use pelo menos 10 caracteres.':primeiroAcesso?'Você será o proprietário da marcenaria principal.':modo==='cadastro'?'Cadastre sua empresa e confirme o e-mail para entrar.':'Entre para acessar seus projetos e orçamentos.'}</p>
        ...
        {modo==='cadastro' && <label>Ramo de atividade<select value={form.segmento} onChange={e=>setForm({...form,segmento:e.target.value})}><option value="outros">Outros ramos</option><option value="comercio">Comércio</option><option value="servicos">Prestação de serviços</option><option value="marcenaria">Marcenaria</option></select></label>}
        {(primeiroAcesso||modo==='cadastro'||modo==='esqueci') ? <label>E-mail<input type="email" autoComplete="email" required value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="voce@empresa.com.br" /></label>
        ...
        <button className="login-botao" disabled={estado.enviando || primeiroAcesso === null}>{estado.enviando ? 'Aguarde…' : primeiroAcesso||modo==='cadastro'?'Criar cadastro':modo==='esqueci'?'Enviar recuperação':modo==='redefinir'?'Salvar nova senha':'Entrar no painel'}<span>→</span></button>
